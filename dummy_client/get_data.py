...
def main():
    ...
    set_display.lcd_init()
    sensor = DHT('11', 14)

    while True:
        humi, temp = sensor.read()
        text = 'humi {0:.1f}%'.format(humi)
        text2 = 'temp {0:.1f}C'.format(temp)
        if humi is not None:
            set_display.lcd_string(text, '1')
            set_display.lcd_string(text2, '2')
            print('DHT{0}, humidity {1:.1f}%, temperature {2:.1f}C'.format(sensor.dht_type, humi, temp))
        else:
            set_display.lcd_string(text2, '1')
